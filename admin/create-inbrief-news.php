...
$title_Err=$image_Err=$slug_Err=$author_Err=$description_Err= "";
...
// Validate description
  ...
  if(isset($_FILES["photo"]) && $_FILES["photo"]["error"] == 0){
    ...
    if(empty($title_Err) && empty($slug_Err) && empty($author_Err)  && empty($description_Err)){
        ...
        $sql = "INSERT INTO in_brief (filename, title, slug, author, description) VALUES (?,  ?,?, ?,  ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_filename, $param_title,  $param_slug, $param_author, $param_description);
