 ...
 $input_url = trim($_POST["url"]);
  ...
  if(isset($_FILES["photo"]) && $_FILES["photo"]["error"] == 0){
    ...
    if(empty($title_Err) && empty($url_Err) && empty($position_Err)){
        ...
        $sql = "INSERT INTO advertisement (filename, title, url,category, position) VALUES (?,  ?, ?,?,  ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_filename, $param_title, $param_url, $param_category, $param_position);
...
<div class="nav-container">
  ...
  <a href="advertisement.php" class="btn btn-success">Back to List</a>
      ...
      <select id="category" name="category" onchange="populate(this.id,'position')">
          ...
          <option value="Small_ad">Small Ad</option>
          <option value="Big_ad">Big Add</option>
